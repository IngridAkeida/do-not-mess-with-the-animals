  ...
  return (
    ...
    <div className='flex gap-4 justify-center'>
      ...
      {results.map((result, index) => {
        ...
        return (
        <li key={index} className='p-2 flex flex-col sm:flex-row items-center justify-center gap-4 bg-dark-primary-a40 rounded-md shadow-md w-60 sm:w-screen sm:h-40 relative sm:mr-4'>
        <div className='block sm:hidden'>{result.backgroundImage=== null ? (
            <Image className={stylesPosterMobile} src='/assets/movie-nf.png' width={300} height={300} alt={result.name} />
          ) : (
            <Image className={stylesPosterMobile} src={`https://www.doesthedogdie.com/content/200/0/${result.backgroundImage}`} width={300} height={300} alt={result.name} />
          )}
        </div>
        <div className='hidden sm:block w-1/12'>{result.posterImage=== null ? (
            <Image className={stylesPosterWeb} src='/assets/movie-nf.png' width={500} height={500} alt={result.name} />
          ) : (
            <Image className={stylesPosterWeb} src={`https://www.doesthedogdie.com/content/200/0/${result.posterImage}`} width={500} height={500} alt={result.name} />
          )}
        </div>
